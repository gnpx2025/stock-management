...
**Constraints**: Constitution v2.0.0 stack and boundaries; PrimeNG-only UI kit; no competing UI libraries; no business entities/APIs/Login; secrets never committed; Shell owns layout/theme/loading/notifications; independently buildable MFE path via Native Federation
...
| Stack: Angular/Nx/Native Federation/PrimeNG/SCSS + .NET 10/Clean Architecture/EF Core/PostgreSQL | Pass | Spec + plan aligned |
...
| Shared UI + PrimeNG-first; no competing UI libs; no unnecessary wrappers | Pass | FR-006/010 |
...
| PrimeNG-first; no competing UI / wrappers | Pass | `research.md` §3 |
...
│   ├── ui/                        # PrimeNG theme wiring, shared UI styles, minimal shared UI
